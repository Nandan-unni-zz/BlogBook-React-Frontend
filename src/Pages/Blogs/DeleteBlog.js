...
import { deleteBlogAPI, getBlogAPI } from '../../Services/BlogServices';
  ...
  render() {
    ...
    return (
      <div className="DeleteAccount">
        ...
        { this.state.user.username === blog.author.username ? <div>
        <Logo></Logo><br /><br /><br />
        <Portal>
          <Form onFinish={this.handleSubmit}>
            <center><h2>Delete Blog</h2></center><br />
            <p>Are you sure you want to delete the blog '<l>{blog.title}</l>' ?</p><br/>
            <Form.Item>
            <center>{!this.state.isSuccess ? <err>{this.state.errMsg}</err> : <Redirect to={`/writer/view/${this.state.user.username}`} />}</center>
            <br />
              <center>
